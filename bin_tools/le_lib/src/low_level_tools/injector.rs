...
use super::compiler::CompilationResult;
...
pub unsafe fn inject_hook(
    ...
    let hook_function_address = match crate::hook_tools::get_function_address(&hook.hook_function) {
        Ok(addr) => addr,
...
unsafe fn write_memory(address: u64, data: &[u8]) -> Result<(), String> {
    ...
    let mut old_protection: libc::c_int = 0;
