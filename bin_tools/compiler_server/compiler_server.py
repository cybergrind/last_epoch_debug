    ...
    def do_POST(self):
        ...
        if self.path == '/compile':
            ...
            if format not in valid_formats:
                self._send_error_response(400, f"Invalid format. Must be one of: {', '.join(valid_formats)}")
                return
            ...
            try:
                ...
                logger.info(f"Compilation successful: {len(binary_data)} bytes in {elapsed_time:.2f}s")
...
def main():
    ...
    try:
        subprocess.run(['nasm', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        logger.error("NASM is not installed or not in PATH. Please install NASM to use this server.")
        exit(1)
